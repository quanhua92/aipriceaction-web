...
import {
	ALL_WATCHLIST_NAME,
	BASIC_WATCHLIST_PREFETCH_COUNT,
...
export function BasicWatchList({
	...
	const handleSortedTickersChange = React.useCallback(
		(newSortedTickers: Ticker[]) => {
			const current = sortedTickersRef.current;
			// Only update if the sorted tickers have actually changed
			if (
				newSortedTickers.length !== current.length ||
				newSortedTickers.some(
					(ticker, index) => ticker.symbol !== current[index]?.symbol,
				)
			) {
	...
	const handleExport = () => {
		try {
			...
			info(
				"Watchlists",
				`Exported ${watchlistCount} watchlist${watchlistCount !== 1 ? "s" : ""}`,
			);
		} catch (err) {
	...
	const handleFileChange = async (
		...
		try {
			...
			// Validate structure
			if (
				typeof importedWatchlists !== "object" ||
				importedWatchlists === null ||
				Array.isArray(importedWatchlists)
			) {
				throw new Error(
					"Invalid file format: expected object with watchlist names as keys",
				);
			}
			...
			for (const [name, tickers] of Object.entries(importedWatchlists)) {
				if (typeof tickers !== "string") {
					throw new Error(
						`Invalid watchlist format: "${name}" should have comma-separated tickers as string`,
					);
				}
			}
			...
			const overwrittenCount = importedNames.filter((name) =>
			...
			info(
				"Watchlists",
				`Imported ${importedNames.length} watchlist${importedNames.length !== 1 ? "s" : ""} (${newCount} new, ${overwrittenCount} overwritten)`,
			);
			...
			logError(
				"Watchlists",
				`Failed to import watchlists: ${err instanceof Error ? err.message : "Unknown error"}`,
			);
		} finally {
			// Reset file input
	...
	return (
		<div className={`flex flex-col ${className}`}>
			...
			<div className="flex items-center mb-3 shrink-0 px-3">
				<h3 className="text-lg font-semibold">{t("common.watchlist.title")}</h3>
			</div>
			...
			{!loading &&
				...
				!globalLoading &&
				!globalError &&
				priceDistribution.totalTickers > 0 && (
				...
				<div className="px-3 mb-2">
					<div className="relative">
						...
						<Input
							...
							className="pl-8 h-7 text-xs bg-muted/50 border-0 focus-visible:ring-1"
						/>
					</div>
				</div>
			)}
								...
								: // Custom watchlist: only show crypto symbols that are IN the watchlist
									...
									? (() => {
											const watchlistSymbols =
												getWatchlistTickers(selectedGroup);
											return cryptoTickers.filter((ticker) =>
							...
							: // GLOBAL: show all global
								...
								: // Custom watchlist: only show global symbols that are IN the watchlist
									...
									? (() => {
											const watchlistSymbols =
												getWatchlistTickers(selectedGroup);
											return globalTickers.filter((ticker) =>
												watchlistSymbols.includes(ticker.symbol),
											);
										})()
