...
import {
	type ChartCompareState,
	getCompareState,
	saveCompareState,
} from "@/lib/compare-storage";

export type TitleGeneratorCallback = (
	ticker: string,
...
export function ChartFullscreenDialog({
	...
	const getTickersToPrefetch = React.useCallback(
		(
			fromIndex: number,
			direction: "next" | "prev",
			count: number,
	...
	const navigateToPrevious = React.useCallback(() => {
		if (!tickerList || tickerList.length === 0) return;
		const newIndex =
			internalIndex === 0 ? tickerList.length - 1 : internalIndex - 1;
		setInternalIndex(newIndex);
	...
	React.useEffect(() => {
		...
		const timer = setTimeout(() => {
			...
			prefetchTickers("ChartFullscreenDialog.initial", [
				...nextSymbols,
				...prevSymbols,
			]);
		}, 100);
			...
			<DialogContent className="sm:max-w-[98vw] max-w-[98vw] w-[98vw] h-[90vh] h-[90dvh] p-2 gap-2 flex flex-col">
				...
				{internalTicker && (
					<Tabs
						...
						onValueChange={(value) =>
							setActiveTab(
								value as "chart" | "compare" | "trendSignal" | "aiContext",
							)
						}
						className="flex-1 min-h-0 flex flex-col gap-0"
					>
						<TabsList className="mx-auto mb-2 gap-1 overflow-x-auto h-auto">
							<TabsTrigger value="chart">
								{t("dialogs.quickAddAlert.tabs.chart")}
							</TabsTrigger>
							<TabsTrigger value="compare">
								{t("dialogs.quickAddAlert.tabs.compare")}
							</TabsTrigger>
							<TabsTrigger value="trendSignal">
								{t("dialogs.quickAddAlert.tabs.trendSignal")}
							</TabsTrigger>
							<TabsTrigger value="aiContext">
								{t("dialogs.quickAddAlert.tabs.aiContext")}
							</TabsTrigger>
						</TabsList>
						...
						<TabsContent
							value="chart"
							className="flex-1 min-h-0 overflow-hidden pb-2"
						>
							{activeTab === "chart" && (
						...
						>
							{activeTab === "aiContext" && (
								<AIContextTab ticker={internalTicker} endDate={endDate} />
							)}
						</TabsContent>
					</Tabs>
				)}
