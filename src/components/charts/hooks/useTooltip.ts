...
import { formatPrice, formatPercent, formatVolume, parseUTCISOString } from "@/lib/format";
...
export const useTooltip = (chartContainerRef: React.RefObject<HTMLDivElement>) => {
  ...
  const buildTooltipHTML = useCallback((
    ...
    paramTime: any,
    tickerSymbol: string,
    maVisibility: { ma10: boolean; ma20: boolean; ma50: boolean; ma100: boolean; ma200: boolean },
    maData: Record<string, { value: number } | undefined>
  ): string => {
    const dateStr = formatDateTime(paramTime);

    // Price and volume change percentages
    const priceChangePercent = currentDataPoint.close_changed !== null && currentDataPoint.close_changed !== undefined
    ...
    const volumeChangePercent = currentDataPoint.volume_changed !== null && currentDataPoint.volume_changed !== undefined
    ...
    // Add moving averages
    const maConfigs = [
      { key: 'ma10', label: 'MA10', color: '#dc2626' },
      { key: 'ma20', label: 'MA20', color: '#16a34a' },
      { key: 'ma50', label: 'MA50', color: '#2563eb' },
      { key: 'ma100', label: 'MA100', color: '#a1a1aa' },
      { key: 'ma200', label: 'MA200', color: '#71717a' },
    ];
    ...
    const visibleMAs = maConfigs.filter(config => {
      ...
      const hasScore = currentDataPoint[`${config.key}_score` as keyof StockData] !== null;
  ...
  const positionTooltip = useCallback((position: TooltipPosition) => {
    if (!tooltipRef.current || !chartContainerRef.current) return;

    if (position === 'top-right') {
      const left = chartContainerRef.current.clientWidth - TOOLTIP_WIDTH - TOOLTIP_MARGIN;
  ...
  const showTooltip = useCallback((
    ...
    const content = buildTooltipHTML(dataPoint, paramTime, tickerSymbol, maVisibility, maData);
    if (!content) return;
  ...
  const showTooltipAtPosition = useCallback((
    ...
    showTooltip(mode, dataPoint, paramTime, tickerSymbol, maVisibility, maData, { x, y });
  }, [showTooltip]);
  ...
  const cleanup = useCallback(() => {
    if (tooltipRef.current && chartContainerRef.current && tooltipRef.current.parentNode === chartContainerRef.current) {
