...
const DataLine = ({ data }: { data: StockData }) => {
  ...
  const isIntradayInterval = INTRADAY_INTERVALS.includes(data.interval as Interval);
  ...
  return (
    <>
      ...
      <span className={cn(priceColor)}>
        {formatPrice(data.close, data)}
      </span>{" "}
      <span className={cn(percentColor)}>
        {formatPercent(change)}
      </span>
      <span className="mx-1"></span>
      <span className={cn((data.volume_changed ?? 0) >= 0 ? "text-green-400" : "text-red-400")}>
        {formatVolume(data.volume)}
      </span>{" "}
      <span className={cn((data.volume_changed ?? 0) >= 0 ? "text-green-600" : "text-red-600")}>
...
export const ChartOverlay = ({ data, maVisibility, interval }: ChartOverlayProps) => {
  if (!data) return null;
  ...
  const maValues = useMemo(() => {
    ...
    if (maVisibility.ma10 && data.ma10 != null) {
      values.push({ label: 'MA10', value: data.ma10, color: '#dc2626' });
    }
    if (maVisibility.ma20 && data.ma20 != null) {
      values.push({ label: 'MA20', value: data.ma20, color: '#16a34a' });
    }
    if (maVisibility.ma50 && data.ma50 != null) {
      values.push({ label: 'MA50', value: data.ma50, color: '#2563eb' });
    }
    if (maVisibility.ma100 && data.ma100 != null) {
      values.push({ label: 'MA100', value: data.ma100, color: '#a1a1aa' });
    }
    if (maVisibility.ma200 && data.ma200 != null) {
      values.push({ label: 'MA200', value: data.ma200, color: '#71717a' });
    }
    ...
    <div>
      <div
        className={cn(
          ...
          data.symbol && data.symbol.length >= 4 ? "text-[11px]" : "text-[12px]"
        )}
        style={{
          WebkitFontSmoothing: "antialiased",
          MozOsxFontSmoothing: "grayscale",
