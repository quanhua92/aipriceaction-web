...
export function PlaygroundChart() {
	...
	const currentStartDate =
		visibleData.length > 0 ? visibleData[0]?.time?.split("T")[0] : null;
	...
	const overlay = useMemo(() => {
		...
		for (const order of orders) {
			...
			markers.push({
				time: entryTime,
				position:
					order.side === "long" ? ("belowBar" as const) : ("aboveBar" as const),
				shape:
					order.side === "long" ? ("arrowUp" as const) : ("arrowDown" as const),
				color: order.side === "long" ? "#22c55e" : "#ef4444",
				text: `${order.side === "long" ? "L" : "S"} ${order.entryPrice.toFixed(2)}`,
			});
		...
		// Sort markers by time ascending (required by SDK)
	...
	const secondaryChartProps = {
		...
		cacheMetadata: {
			...
			startDate:
				secondaryVisibleData.length > 0
					? secondaryVisibleData[0]?.time?.split("T")[0] || ""
					: "",
			endDate:
				secondaryVisibleData.length > 0
					? secondaryVisibleData[secondaryVisibleData.length - 1]?.time?.split(
							"T",
						)[0] || ""
					: "",
	...
	return (
		<div>
			...
			>
				{shouldShowSecondary && <TradingViewChart {...secondaryChartProps} />}
				<TradingViewChart {...primaryChartProps} />
			</div>
		</div>
	);
