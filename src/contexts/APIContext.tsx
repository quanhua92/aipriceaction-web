...
import {
	...
	getHealth as getHealthApi,
	getTickerGroups,
	getTickerNamesWithLogging,
	getTickersWithLogging,
	getYahooTickerGroups,
	getYahooTickersWithLogging,
	type HealthResponse,
	type StockData,
	type TickerGroups,
	type TickerNames,
...
const sortTickerGroups = (groups: TickerGroups): TickerGroups => {
	const sortedEntries = Object.entries(groups).sort(([groupA], [groupB]) => {
		const priorityA = PRIORITY_GROUPS.indexOf(
			groupA as (typeof PRIORITY_GROUPS)[number],
		);
		const priorityB = PRIORITY_GROUPS.indexOf(
			groupB as (typeof PRIORITY_GROUPS)[number],
		);
...
export function APIProvider({ children }: { children: React.ReactNode }) {
	...
	// VN stocks state
	const [tickerGroups, setTickerGroups] = React.useState<TickerGroups | null>(
		null,
	);
	const [tickers, setTickers] = React.useState<Ticker[]>([]);
	const [allTickersLastData, setAllTickersLastData] = React.useState<
		Record<string, BasicStockData[]>
	>({});
	const [stockLoading, setStockLoading] = React.useState(true);
	const [stockError, setStockError] = React.useState<string | null>(null);

	// Crypto state
	const [cryptoTickerGroups, setCryptoTickerGroups] =
		React.useState<TickerGroups | null>(null);
	const [cryptoTickers, setCryptoTickers] = React.useState<Ticker[]>([]);
	const [allCryptoTickersLastData, setAllCryptoTickersLastData] =
		React.useState<Record<string, BasicStockData[]>>({});
	const [cryptoLoading, setCryptoLoading] = React.useState(true);
	const [cryptoError, setCryptoError] = React.useState<string | null>(null);

	// Global (Yahoo) state
	const [globalTickerGroups, setGlobalTickerGroups] =
		React.useState<TickerGroups | null>(null);
	const [globalTickers, setGlobalTickers] = React.useState<Ticker[]>([]);
	const [allGlobalTickersLastData, setAllGlobalTickersLastData] =
		React.useState<Record<string, BasicStockData[]>>({});
	const [globalLoading, setGlobalLoading] = React.useState(true);
	const [globalError, setGlobalError] = React.useState<string | null>(null);

	// Ticker names state
	const [tickerNames, setTickerNames] = React.useState<TickerNames | null>(
		null,
	);
	const [cryptoTickerNames, setCryptoTickerNames] =
		React.useState<TickerNames | null>(null);
	const [globalTickerNames, setGlobalTickerNames] =
		React.useState<TickerNames | null>(null);

	// EMA toggle state
	const [ema, setEma] = React.useState<boolean>(() => {
		const saved = SafeLocalStorage.getItem(EMA_STORAGE_KEY);
		return saved === null ? true : saved === "true";
	});

	React.useEffect(() => {
		SafeLocalStorage.setItem(EMA_STORAGE_KEY, ema ? "true" : "false");
	}, [ema]);
	...
	const fetchStockData = React.useCallback(async () => {
		...
		try {
			...
			const [groups, tickersData] = await Promise.all([
				getTickerGroups(),
				getTickersWithLogging(
					"APIContext.init.stock",
					{ limit: 1, end_date: globalEndDate, ma: false },
					{ info, warn: info, error: logError },
				),
			]);
			...
			const errorMessage =
				err instanceof Error ? err.message : "Failed to load stock market data";
	...
	const fetchTickerNames = React.useCallback(async () => {
		try {
			const [vnNames, cryptoNames, globalNames] = await Promise.all([
				getTickerNamesWithLogging("APIContext.names.vn", "vn", {
					info,
					warn: info,
					error: logError,
				}),
				getTickerNamesWithLogging("APIContext.names.crypto", "crypto", {
					info,
					warn: info,
					error: logError,
				}),
				getTickerNamesWithLogging("APIContext.names.global", "yahoo", {
					info,
					warn: info,
					error: logError,
				}),
	...
	const getTickers = React.useCallback(
		async (source: string, params?: TickersQueryParams) => {
			...
			const promise = getTickersWithLogging(source, params ?? {}, {
				info,
				warn: info,
				error: logError,
			}).then((result) => result);
	...
	const getHealth = React.useCallback(
		async (source: string) => {
			try {
				const startTime = Date.now();
				...
				const duration = Date.now() - startTime;
				info(
					`[API] ${source} getHealth: ${duration}ms | Active tickers: ${health.active_tickers_count}`,
				);
				return health;
			} catch (err) {
				const errorMessage = err instanceof Error ? err.message : String(err);
