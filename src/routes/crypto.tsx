...
import {
	CRYPTO_CHART_TICKERS_STORAGE_KEY,
...
function CryptoPage() {
	...
	>(() => {
		const stored = SafeLocalStorage.getItem("crypto-treemap-order");
	...
	const handleSwapTreemapPosition = React.useCallback(() => {
		setTreemapOrder((prev) => {
			...
			SafeLocalStorage.setItem("crypto-treemap-order", next);
			return next;
		});
	}, []);
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex flex-col">
				...
				>
					<TradingTreemap
						defaultWatchlist={CRYPTO_WATCHLIST_NAME}
